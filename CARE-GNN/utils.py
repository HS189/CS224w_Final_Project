...
def test_care(test_cases, labels, model, batch_size):
	...
	for iteration in range(test_batch_num):
		...
		f1_gnn += f1_score(batch_label, gnn_prob.data.cpu().numpy().argmax(axis=1), average=avg)
		acc_gnn += accuracy_score(batch_label, gnn_prob.data.cpu().numpy().argmax(axis=1))
		recall_gnn += recall_score(batch_label, gnn_prob.data.cpu().numpy().argmax(axis=1), average=avg)

		f1_label1 += f1_score(batch_label, label_prob1.data.cpu().numpy().argmax(axis=1), average=avg)
		acc_label1 += accuracy_score(batch_label, label_prob1.data.cpu().numpy().argmax(axis=1))
		recall_label1 += recall_score(batch_label, label_prob1.data.cpu().numpy().argmax(axis=1), average=avg)
...
def modified_test_care(test_cases, labels, model, thresh=0.5):
	gnn_prob, label_prob1 = model.to_prob(test_cases, labels, train_flag=False)
	pred_proba = gnn_prob.data.cpu().numpy()
	print(pred_proba.shape)
	# preds = pred_proba.argmax(axis=1)
	preds = (pred_proba[:,1] > thresh).astype(int)

	f1 = f1_score(labels, preds, average='binary')
	prec = precision_score(labels, preds, average='binary')
	rec = recall_score(labels, preds, average='binary')
	auc_gnn = roc_auc_score(labels, np.array(pred_proba[:,1]))

	print('my f1:', f1)
	print('my precision:', prec)
	print('my recall:', rec)
	print('my auc:', auc_gnn)

def modified_test_sage(test_cases, labels, model):
	gnn_prob = model.to_prob(test_cases)
	pred_proba = gnn_prob.data.cpu().numpy()
	print(pred_proba.shape)
	preds = pred_proba.argmax(axis=1)
...
def best_val_thresh_care(test_cases, labels, model):
	gnn_prob, label_prob1 = model.to_prob(test_cases, labels, train_flag=False)
	pred_proba = gnn_prob.data.cpu().numpy()
	y_score = pred_proba[:,1]
	precision, recall, thresholds = precision_recall_curve(labels, np.array(y_score))
